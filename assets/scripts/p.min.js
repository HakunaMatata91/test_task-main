            ...
            $("#content2").fadeIn(), setTimeout(function () {
                ...
                $("#content2").fadeOut("slow", function () {
                    "undefined" != typeof roulette_ini
                        ? rouletteRoot._init() : "undefined" != typeof box_ini && boxRoot._init(), $("#content3").fadeIn()
                })
            }, 7100)
        })
    })
...
// Adding an event handler for the click event on the "OK" button
$("#p_modal_button3").click(function (event) {
    event.preventDefault(); // Canceling the default link action

    // Getting the selected user responses
    var selectedResponses = {};

    // Iterating through each question and checking which answer is selected
    $("[id^='q']").each(function () {
        var questionId = $(this).attr("id");
        var selectedAnswer = $(this).find(".survey_button.selected[data-bq]").text(); // Getting the text of the selected answer

        if (selectedAnswer) {
            selectedResponses[questionId] = selectedAnswer;
        }

    });

    // Displaying the selected responses as a single string
    var responseString = "";
    Object.keys(selectedResponses).forEach(function (questionId) {
        responseString += `${selectedResponses[questionId].toString()}`;
    });
    console.log(`User responses: ${responseString}`);

    // Additionally, you can call a function to track the event if needed
...
// Adding an event handler for the click event on answer buttons
$(".survey_button").click(function () {
    $(this).toggleClass("selected"); // Toggling the "selected" class on click
});
